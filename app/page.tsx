...
export default async function DashboardPage() {
  const [
    ...
    supabaseAdmin
      ...
      .in('status', ['lead', 'scheduled', 'in_progress']),
    supabaseAdmin.from('invoices').select('amount_cents').eq('status', 'succeeded'),
  ]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card">
          ...
          <p className="text-2xl font-bold">{customers ?? 0}</p>
        </div>
        <div className="card">
          <p className="text-sm text-slate-500">Jobs</p>
          <p className="text-2xl font-bold">{jobs ?? 0}</p>
        </div>
        <div className="card">
          <p className="text-sm text-slate-500">Open Jobs</p>
          <p className="text-2xl font-bold">{openJobs ?? 0}</p>
        </div>
        <div className="card">
